...
export default async (req, res) => {
  ...
  const user = await User.findOne({ _id: req.user._id, isActivated: true });

  if (!user) return res.status(400).json(errorHelper('00052', req));

  if (req.body.code !== req.user.code){
    return res.status(400).json(errorHelper('00054', req));
  }

  // Update user's isVerified field to true only if the verification code matches
  if (!user.isVerified) {
  ...
  const expires = new Date(Date.now() + 604800000); // 7 days expiry
  ...
  return res.status(200).json({
    resultMessage: { en: getText('en', '00058') },
